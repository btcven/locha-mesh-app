...
char *buscar(char *query, sqlite3 *db)
{
  ...
  {
    resp = (char *)sqlite3_column_text(res, 1); // number 1 is column number (field) on query
    if (rec_count > 5000)
    {
...
// exec a select query and return only first record/first field
// if select return more than 10000 records then return empty  (this function is only used for small tables or recordsets)
/* it uses: int sqlite3_prepare_v2(
  sqlite3 *db,             Database handle
  const char *zSql,        SQL statement, UTF-8 encoded
  int nByte,               Maximum length of zSql in bytes.
...
int buscar_valor(const char *query, sqlite3 *db)
{
  ...
  if (rc != SQLITE_OK) {
    ESP_LOGE(TAG, "Failed to fetch data:%s", sqlite3_errmsg(db));
    return 0;   // TODO: check if error received returns NULL instead of zero
  }

  while (sqlite3_step(res) == SQLITE_ROW) {
    resp = sqlite3_column_int(res, 0);
    ESP_LOGE(TAG, "received data: %d", resp);
    rec_count++;
  }
...
esp_err_t SQLite_INIT()
{
  ...
  // test if database is working OK: first insert, next select
  // insert dummy for checking database
  ...
  // ESP_LOGE(TAG, "received data:%s",rpta2);
