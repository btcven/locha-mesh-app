 ...
 */
 ...
 void process_received_packet(const char id_node_from[SIZE_IDNODE], packet_t packet_temporal, int RSSI_recibido, int SNR_recibido, sqlite3 *db){
  uint8_t rpta;
  char *pChar = (char*)"";
  const char *TAG = "process_received_packet";
  char *id_node_const;
  strcpy(id_node_const,id_node_from);
  ...
  // solo se procesa un packet si es para mi (to,next_neighbour) o si es un broadcats (to="")
      ...
      // si el packet es para mi nodo o es un broadcast
       if ((compare_char(packet_temporal.header.to,id_node_const))or((compare_char(packet_temporal.header.to,pChar)))or((compare_char(packet_temporal.header.next_neighbor,id_node_const)))){
        ...
        ESP_LOGE(TAG, "A new packet incoming direct for my node, type:%s",convertir_packet_type_e_str(packet_temporal.header.packet_type));
            ...
            case EMPTY:
              ...
              break;
            case DATA:
                protocol_incoming_PACKET_DATA(id_node_from, packet_temporal);
                break;
            case ROUTING:
                protocol_incoming_PACKET_ROUTING(id_node_from, packet_temporal,RSSI_recibido, SNR_recibido,db);
                break;
            case SECURITY:
                protocol_incoming_PACKET_SECURITY(id_node_from, packet_temporal);
                break;
            default:
              break;
            }
