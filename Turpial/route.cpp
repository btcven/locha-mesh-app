...
void devolver_como_packet_not_delivered(packet_t paquet_in_process2,not_delivered_type_e motivo){
    // se invierte el remitente con el destinatario
      ...
      why_not_delivered=motivo;
        ...
        uint8_t rptas=packet_to_send(paquet_in_process2,mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
 ...
 //   DEBUG_PRINT(i);
  encontre=false;
      i=1;
        ...
        if (mensajes_salientes[i].paquete.header.type!=EMPTY){
        msg_to_send=packet_serialize(mensajes_salientes[i].paquete);
        bool show_interactive=true;
        if ((mensajes_salientes[i].paquete.header.type==HELLO)){
          show_interactive=false;
        }
        if ((mensajes_salientes[i].paquete.header.type==JOIN)){
          show_interactive=false;
        }
        uint8_t rpta_rad=radioSend(msg_to_send,show_interactive);
        ...
        if (rpta_rad==0){ 
          uint8_t rptas=packet_to_send(mensajes_salientes[i].paquete,mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
        }
       ...
       mensajes_salientes[total_mensajes_salientes].paquete.header.type=EMPTY;
       total_mensajes_salientes--;
...
// update age of a route in routeTable , if didnt exist create route
...
uint8_t packet_to_send(packet_t packet_temp, message_queue_t (&mensajes_salientes)[MAX_MSG_QUEUE], uint8_t &total_mensajes_salientes, uint8_t total_vecinos, uint8_t total_rutas){
  ...
  if (total_vecinos>0){
          ...
          if (!exist_route){
            if (!compare_char(packet_temp.header.to,"")){
            ...
            if (!(compare_char(vecino_mas_conectado,""))){
                DEBUG_PRINTLN(F("no es vacio, se coloca como el next neigbour que se va a encargar del packet"));
              ...
              DEBUG_PRINTLN(F("No hay vecino mas conectado"));
            }
            
          }
          }
          // se coloca en la cola de mensajes salientes
          ...
          nuevo_mensaje_en_cola.retry_timestamp=millis();
           if (total_mensajes_salientes<=MAX_MSG_QUEUE){
               ...
               mensajes_salientes[total_mensajes_salientes]=nuevo_mensaje_en_cola;
              
             
              DEBUG_PRINTLN(F("Se coloco en cola:"));
            ...
            DEBUG_PRINTLN(F("MSG QUEUE FULL"));
            return 1;
          }
        
        if (send_route_request){
          // TODO: se envian solicitudes de info de ruta a los demas nodos vecinos, pero solo si packet origen distinto a id_node
        }
  } else {
    // Cualquier packet que no sea HELLO si trata de enviarse y no hay vecinos se envia el mensaje de error
    if (packet_temp.header.type!=HELLO){
      ...
      // los HELLO se mandan directo a la cola de salida
          message_queue_t nuevo_mensaje_en_cola;
          nuevo_mensaje_en_cola.paquete=packet_temp;
          nuevo_mensaje_en_cola.prioridad=1;
          nuevo_mensaje_en_cola.retries=0;
...
void broadcast_packet(char* id_node,packet_type_e tipo_de_packet, char* hash, char* payload, struct nodo_t (vecinos)[MAX_NODES], uint8_t total_vecinos, message_queue_t (&mensajes_salientes)[MAX_MSG_QUEUE], uint8_t &total_mensajes_salientes, uint8_t total_rutas){
    ...
    for (i = 1; i <= total_vecinos; i++) {         
            new_packet=create_packet(id_node, tipo_de_packet,id_node , vecinos[i].id, "",hash,payload);
            rpta=packet_to_send(new_packet,mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);
    }
}
...
void broadcast_bye(char* id_node, struct nodo_t (vecinos)[MAX_NODES], uint8_t total_vecinos, message_queue_t (&mensajes_salientes)[MAX_MSG_QUEUE], uint8_t &total_mensajes_salientes, uint8_t total_rutas){
    ...
    broadcast_packet(id_node,BYE,"","",vecinos,total_vecinos,mensajes_salientes,total_mensajes_salientes,total_rutas);
}
