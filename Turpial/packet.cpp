...
packet_t create_packet(packet_type_e tipo_packet, subtype_u subtipo_packet, const char *from, const char *to, const char *next_neighbor, char *checksum_data, char *payload, uint16_t packet_number, uint16_t packet_total)
{
  ...
  header.from = strdup(from);
  header.to = strdup(to);
  header.next_neighbor = strdup(next_neighbor);
  header.checksum_data = strdup(checksum_data);
  header.timestamp = millis();

  body_data_u body;


  // payload depends of packet_t
  if (tipo_packet == DATA) {
    ...
    body.body_data_splitted.payload = strdup(payload);
  } else {
    body.body_data.payload_length = sizeof(body.body_data.payload);
    body.body_data.payload = strdup(payload);
  }
  ...
  new_packet.body = body;
    ...
    free(header.from);
    free(header.to);
    free(header.next_neighbor);
    free(header.checksum_data);
  if (tipo_packet == DATA) {  
    free(body.body_data_splitted.payload);
  } else {
    free(body.body_data.payload);
  }
  return new_packet;
}
...
// converts a char* into typedef enum packet_type_e
