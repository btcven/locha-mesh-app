...
packet_type_e convertir_str_packet_type_e(char *type_recibido)
{
  ...
  if (compare_char(type_recibido, pChar))
    return EMPTY;
  pChar = (char *)"ACK";
  if (compare_char(type_recibido, pChar))
    return ACK;
...
char *convertir_packet_type_e_str(packet_type_e type_recibido)
{
  char *rpta = NULL;
  char *pChar = (char *)"EMPTY";
  if (type_recibido == EMPTY)
    rpta = pChar;
  pChar = (char *)"ACK";
  if (type_recibido == ACK)
    rpta = pChar;
  pChar = (char *)"JOIN";
  if (type_recibido == JOIN)
    rpta = pChar;
  pChar = (char *)"BYE";
  if (type_recibido == BYE)
    rpta = pChar;
  pChar = (char *)"MSG";
  if (type_recibido == MSG)
    rpta = pChar;
  pChar = (char *)"ROUTE";
  if (type_recibido == ROUTE)
    rpta = pChar;
  pChar = (char *)"HELLO";
  if (type_recibido == HELLO)
    rpta = pChar;
  pChar = (char *)"GOSSIP";
  if (type_recibido == GOSSIP)
    rpta = pChar;
  pChar = (char *)"NOT_DELIVERED";
  if (type_recibido == NOT_DELIVERED)
    rpta = pChar;
