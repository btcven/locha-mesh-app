        ...
        // se devuelve un packet_ACK por la misma ruta al origen para notificar la recepcion
            ...
            DEBUG_PRINT(F("Se esta procesando routing_incoming_PACKET_MSG:"));
             ...
             //   String hacia_el_ble=Json_return_msg((String)packet_received.body.payload);
              ...
              DEBUG_PRINT(hacia_el_ble.c_str());
              DEBUG_PRINT(F("-largo del mensaje enviado:"));
              DEBUG_PRINT((String)hacia_el_ble.length());
            
            uint8_t rptas=packet_to_send(new_packet,mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
...
uint8_t routing_incoming_PACKET_ROUTE(char id_node[SIZE_IDNODE], packet_t packet_received){
  ...
  DEBUG_PRINTLN(F("se recibio un packet route"));
  return 0;
}
...
uint8_t routing_incoming_PACKET_NOT_DELIVERED(char id_node[SIZE_IDNODE], packet_t packet_received){
  // si no es para mi se reenvia el paquete a los vecinos por la ruta donde origino
   uint8_t i;
  uint8_t is_MSG=0;
  DEBUG_PRINTLN(F("se recibio un packet not delivered"));
...
DEBUG_PRINTLN(F("se recibio un packet gossip"));
...
uint8_t routing_incoming_PACKET_TXN(char id_node[SIZE_IDNODE], packet_t packet_received){
  // 
  DEBUG_PRINTLN(F("se recibio un packet txn"));
  return 0;
}
...
for (i = 1; i <= total_rutas_blacklist; i++) {
  ...
  // se crea una ruta al packet que envio el HELLO y se devuelve un PACKET_JOIN
   ...
   if (!existe_previamente_vecino){
    DEBUG_PRINTLN(F("se recibio un packet hello"));
   uint8_t rpta1=create_neighbor(packet_received.header.from,vecinos,total_vecinos,blacklist_nodes,total_nodos_blacklist);
...
// se devuelve un packet JOIN
  ...
  DEBUG_PRINTLN(F("Se envia un packet Join"));
 ...
 // se arma como payload el contenido de la tabla vecinos concatenado por el contenido de la tabla rutas
  ...
  // se hace un delay random de hasta 1 segundos ya que el HELLO se origina en modo broadcast y si hay varios nodos asi se trata de minimizar la posibilidad de colision ya que todos van a responder al mismo moento
  int delay_rand=rand() % 1000 + 1;
  delay(delay_rand);
  uint8_t rptax=packet_to_send(packet_received,mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
  ...
  // se le solicita via GOSSIP las rutas que tenga solo para los vecinos que no existan previamente
    ...
    uint8_t rptax2=packet_to_send(new_packet,mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
...
uint8_t routing_incoming_PACKET_ACK(char id_node[SIZE_IDNODE], packet_t packet_received){
  ...
  uint8_t i;
  uint8_t is_MSG=0;
  DEBUG_PRINTLN(F("Packet ACK recibido , se procesa ..."));
   for (i = 1; i <= total_mensajes_waiting; i++) {
    ...
    DEBUG_PRINT("mensajes_waiting:");
    DEBUG_PRINT("To:");
    DEBUG_PRINT(mensajes_waiting[i].paquete.header.to);
    DEBUG_PRINT("-From:");
    DEBUG_PRINT(mensajes_waiting[i].paquete.header.from);
...
// solo se procesa un packet si es para mi (to,next_neighbour) o si es un broadcats (to="")
  ...
  DEBUG_PRINTLN(convertir_packet_type_e_str(packet_temporal.header.type));   
      ...
      case ACK:
          ...
          break;
      case MSG:
         routing_incoming_PACKET_MSG(id_node, packet_temporal,txValue);
         break;
      case HELLO:
      ...
      if (compare_char(packet_temporal.header.next_neighbor,id_node)){
        ...
        for ( jj = 1; jj <= total_rutas;jj++) {
            if (compare_char(routeTable[jj].destino.id,packet_temporal.header.to)){
              ...
              DEBUG_PRINT(F("El destinatario es un vecino de este nodo"));
              encontre_ruta=true;
              route_number=jj;
              uint8_t rptax=packet_to_send(packet_temporal,mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
              break;
            }
        }
        ...
        if (!encontre_ruta){ 
          for ( jj = 1; jj <= total_rutas; jj++) {
                if (compare_char(routeTable[jj].next_neighbor.id,packet_temporal.header.to)){
                    ...
                    DEBUG_PRINT(F("Se envia al next neigbour que conoce este nodo"));
 ...
 // el packet no es para mi nodo y tampoco hice relay sino que escuche un packet de otro nodo no registrado que pudiera ser vecino
 if (!encontre_ruta){
  bool enviar_hello=false;
  uint8_t rptadx;

     if (!(compare_char(packet_temporal.header.from,""))){
       if ((!(es_vecino(packet_temporal.header.from)))and(!(enviar_hello))){
          if (!es_blacklisted(packet_temporal.header.from)){
            // se envia un packet HELLO a ver si responde ese vecino
            rptadx=packet_to_send(construct_packet_HELLO(id_node),mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
            enviar_hello=true;
          }
       }
     }

     if (!(compare_char(packet_temporal.header.to,""))){
       if ((!(es_vecino(packet_temporal.header.to)))and(!(enviar_hello))){
          if (!es_blacklisted(packet_temporal.header.to)){
            // se envia un packet HELLO a ver si responde ese vecino
            rptadx=packet_to_send(construct_packet_HELLO(id_node),mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
            enviar_hello=true;
          }
     ...
     if (!(compare_char(packet_temporal.header.next_neighbor,""))){
       if ((!(es_vecino(packet_temporal.header.next_neighbor)))and(!(enviar_hello))){
          if (!es_blacklisted(packet_temporal.header.next_neighbor)){
            // se envia un packet HELLO a ver si responde ese vecino
            rptadx=packet_to_send(construct_packet_HELLO(id_node),mensajes_salientes,total_mensajes_salientes,total_vecinos, total_rutas);  // se envia a la cola de mensajes salientes
            enviar_hello=true;
          }
